...
class TransformerModelWrapper:
    ...
    def __init__(self, config: WrapperConfig):
        ...
        model_config = config_class.from_pretrained(
            ...
            cache_dir=config.cache_dir if config.cache_dir else None, use_cache=False, mirror='tuna')
        ...
        self.task_helper = TASK_HELPERS[self.config.task_name](self) if self.config.task_name in TASK_HELPERS else None # None
    ...
    def train(self, task_train_data: List[InputExample], device, per_gpu_train_batch_size: int = 8, n_gpu: int = 1,
        ...
        train_dataset = self._generate_dataset(task_train_data) #生成真正的data
        ...
        train_iterator = trange(int(num_train_epochs), desc="Epoch", position=0, leave=True)
    ...
    def mlm_train_step(self, labeled_batch: Dict[str, torch.Tensor],
        ...
        mlm_labels, labels = labeled_batch['mlm_labels'], labeled_batch['labels'] # mlm_labels torch.Size([4, 256]) labels torch.Size(4)

        outputs = self.model(**inputs) # outputs[0]  torch.Size([4, 256, 30522])
