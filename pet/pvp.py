...
class PVP(ABC):
    ...
    def _build_mlm_logits_to_cls_logits_tensor(self):
        ...
        m2c_tensor = torch.ones([len(label_list), self.max_num_verbalizers], dtype=torch.long) * -1 #初始化为-1
    ...
    def convert_mlm_logits_to_cls_logits(self, mlm_labels: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        masked_logits = logits[mlm_labels >= 0] # torch.Size([4, 30522])
        cls_logits = torch.stack([self._convert_single_mlm_logits_to_cls_logits(ml) for ml in masked_logits]) #这里不batch的处理，因为每个样本对应的 verbalize个数可能不同
        return cls_logits

    def _convert_single_mlm_logits_to_cls_logits(self, logits: torch.Tensor) -> torch.Tensor:
        m2c = self.mlm_logits_to_cls_logits_tensor.to(logits.device) # logits torch.Size(30522) m2c torch,Size([4,1]) # [label_size, max_verbalize_size] #初始化为-1
        # filler_len.shape() == max_fillers
        filler_len = torch.tensor([len(self.verbalize(label)) for label in self.wrapper.config.label_list],
                                  dtype=torch.float)
        filler_len = filler_len.to(logits.device) # torch.Size(4)# [label_size, verbalize_size] #有的label 的verbalize比较多，有的label的verbalize较少

        # cls_logits.shape() == num_labels x max_fillers  (and 0 when there are not as many fillers). # 正面情绪可由多个词，比如：`高兴，开心，快乐` 替代。
        cls_logits = logits[torch.max(torch.zeros_like(m2c), m2c)] # m2c中有一些是被填充的-1， 其他的是有真实值的 #torch.max(torch.zeros_like(m2c), m2c)可以将 大于等于0的索引拿到
